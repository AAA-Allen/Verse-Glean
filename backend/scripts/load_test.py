...
async def worker(client: httpx.AsyncClient, stop_at: float, latencies: list, label: str):
    """循环打两类读接口，记录延迟直到截止时间。"""
    i = 0
    while time.time() < stop_at:
        url = f"{BASE}/api/v1/capsules?page=1&page_size=20" if i % 2 == 0 else f"{BASE}/api/v1/graph"
...
async def main_async(args):
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        async with asyncio.TaskGroup() as tg:
            for w in range(args.workers):
                tg.create_task(worker(client, stop_at, latencies, f"w{w}"))
        wall = time.time() - t0
    ...
    report = (
        ...
        f"- 场景：{args.workers} 并发 × {args.seconds}s，混合打 capsules 列表 + graph\n"
