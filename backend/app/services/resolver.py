...
def extract_first_url(share_text: str) -> str | None:
    m = B23_RE.search(share_text) or BILI_BV_RE.search(share_text) or DOUYIN_RE.search(share_text)
...
async def resolve(share_text: str) -> ResolvedVideo:
    ...
    if m := BILI_BV_RE.search(url):
        return await _resolve_bilibili(url, m.group(1))
...
async def _resolve_bilibili(url: str, bvid: str) -> ResolvedVideo:
    """B 站公开 Web API 取元数据（标题/封面/时长），无需登录态。"""
    async with httpx.AsyncClient(timeout=10, headers={"User-Agent": UA, "Referer": "https://www.bilibili.com"}) as client:
        resp = await client.get("https://api.bilibili.com/x/web-interface/view", params={"bvid": bvid})
    ...
    logger.warning("bilibili view api failed: bvid={} status={}", bvid, resp.status_code)
